/*
 * File:   interrup_18k50.c
 * Author: Miguel-Notebook
 *
 * Created on July 15, 2023, 8:03 PM
 */
...
void main(void){
    Int_Ext();
    ANSELD = 0x00; //Puertos digital
    TRISD = 0x00; // Puertos como Salida
    while(1){
        LATD = count;
    }
...
void Int_Ext(void){
    ...
    TRISBbits.RB0 = 1; //es una entrada
    INTCONbits.INT0IE = 1; //se habilita la interrupcion externa
    INTCONbits.INT0IF = 0; //La interrupcion de bandera de desbordamiento esta deshabilitada
...
void __interrupt() INT_isr(void){
    count++;
    INTCONbits.INT0IF = 0; //cuando ejecuta la interrupcion se pone en 1, por eso la volvemos a 0
}
